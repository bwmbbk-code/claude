  ...
  return (
    <div className="bg-white rounded-2xl shadow border border-gray-100 overflow-hidden">
      <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
        ...
        <span className="text-sm text-gray-400">{rounds.length}라운드 (최근 20개 기준)</span>
      </div>
          ...
          <tbody className="divide-y divide-gray-50">
            {sorted.map((round, idx) => {
              ...
              return (
                <tr
                  key={round.id}
                  className={`${isOld ? 'opacity-40' : ''} hover:bg-gray-50 transition-colors`}
                >
                  ...
                  <td className={`px-4 py-3 text-right font-bold ${round.differential <= 0 ? 'text-blue-600' : round.differential >= 10 ? 'text-red-500' : 'text-gray-700'}`}>
                    {round.differential > 0 ? '+' : ''}{round.differential.toFixed(1)}
                  </td>
                  <td className="px-4 py-3 text-center text-gray-400">{course?.rating}</td>
                  <td className="px-4 py-3 text-center text-gray-400">{course?.slope}</td>
                  <td className="px-4 py-3 text-center">
                    <button
                      onClick={() => {
                        if (confirm(`${round.date} 라운드를 삭제할까요?`)) {
                          onDelete(round.id);
                        }
                      }}
